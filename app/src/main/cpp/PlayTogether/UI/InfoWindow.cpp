...
void ShowInfoWindow() {
    ...
    if (ImGui::Begin(OBF("Thong Tin##info_overlay"), nullptr, ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoCollapse)) {
        ...
        ImGui::Text("%s %s", OBF("Thoi Gian Hoat Dong:"), uptime);
        ImGui::Separator();
        ImGui::Text("%s %d", OBF("Cap Do Ca:"), PLConfig::FishingConfig::curFishLevel);
        ImGui::Text("%s %d", OBF("Cap Do Bong Ca:"), PLConfig::FishingConfig::curFishShadowLevel);
        ImGui::Text("%s %d", OBF("Khu Vuc Cau Ca:"), PLConfig::FishingConfig::curFishZone);
        if (gPLConfig.fishing.magicWater.isEnable && FishingSystem::MagicWaterLeft >= 0) {
            ImGui::Text("%s %d", OBF("Buff con lai:"), FishingSystem::MagicWaterLeft);
        }
    }
    ImGui::End();
}
