...
namespace SandHook {
    ...
    public:
        ...
        ElfImg(const std::string& elf);

        constexpr ElfW(Addr) getSymbOffset(const std::string& name) const {
            return getSymbOffset(name, GnuHash(name), ElfHash(name));
        }

        constexpr ElfW(Addr) getSymbAddress(const std::string& name) const {
        ...
        template<typename T>
        constexpr T getSymbAddress(const std::string& name) const {
    ...
    private:
        ElfW(Addr) getSymbOffset(const std::string& name, uint32_t gnu_hash, uint32_t elf_hash) const;

        ElfW(Addr) ElfLookup(const std::string& name, uint32_t hash) const;

        ElfW(Addr) GnuLookup(const std::string& name, uint32_t hash) const;

        ElfW(Addr) LinearLookup(const std::string& name) const;

        constexpr static uint32_t ElfHash(const std::string& name);

        constexpr static uint32_t GnuHash(const std::string& name);
    ...
    constexpr uint32_t ElfImg::ElfHash(const std::string& name) {
        uint32_t h = 0, g = 0;
    ...
    constexpr uint32_t ElfImg::GnuHash(const std::string& name) {
        uint32_t h = 5381;
        for (unsigned char p: name) {
            h += (h << 5) + p;
        }
