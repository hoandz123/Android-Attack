...
inline constexpr auto offsetOf(ElfW(Ehdr) *head, ElfW(Off) off) {
    return reinterpret_cast<std::conditional_t<std::is_pointer<T>::value, T, T *>>(
            reinterpret_cast<uintptr_t>(head) + off);
}

ElfImg::ElfImg(const std::string &base_name) : elf(base_name) {
    if (!findModuleBase()) {
        base = nullptr;
        return;
    }
...
ElfW(Addr) ElfImg::ElfLookup(const std::string &name, uint32_t hash) const {
...
ElfW(Addr) ElfImg::GnuLookup(const std::string &name, uint32_t hash) const {
...
ElfW(Addr) ElfImg::LinearLookup(const std::string &name) const {
    if (symtabs_.empty()) {
...
ElfW(Addr)
ElfImg::getSymbOffset(const std::string &name, uint32_t gnu_hash, uint32_t elf_hash) const {
    ElfW(Addr) offset = GnuLookup(name, gnu_hash);
    if (offset > 0) {
        return offset;
    }
